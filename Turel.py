...
class Turel(pygame.sprite.Sprite):
    ...
    def __init__(self, pos_x, pos_y, frame, walls, hero, all_sprites,
        ...
        self.start_pos = [self.tile_width * pos_x,
                          self.tile_height * pos_y]
        self.rect = self.image.get_rect().move(self.tile_width * pos_x,
                                               self.tile_height * pos_y)
    ...
    def shooting(self, hero, s_x, s_y):
        ...
        if self.distance > 300:
            flag = False
        if flag:
            self.all_sprites.add(Bullet(load_image("."
                                                   "/bullets/enemy_bullet.png",
                                                   -1),
                                        20, 20, (self.rect.x +
                                                 self.rect.w // 2,
                                                 self.rect.y +
                                        ...
                                        (self.hero.rect.x +
                                         ...
                                         self.hero.rect.y +
                                         self.hero.rect.h // 2),
                                        300, self.walls, self.hero,
                                        self.enemies_group))

    def update(self, *args):
        if self.hp <= 0:
            self.all_sprites.add(Coin(self.rect.x / 50, self.rect.y / 50,
                                      self.hero))
